        ...
        try:
            if request.GET.get('advert_id'):
                ...
                advert = Advert.objects.get(advert_id=request.GET.get('advert_id'),
                                            supplier_id=request.GET.get('sub_id'))
...
def my_subscribers_list(request):
    try:
        ...
        try:
            ...
            if sub_city_id != 'all':
                supplier_obj = supplier_obj.filter(city_place_id=sub_city_id)
            if user_id != 'all':
                supplier_obj = supplier_obj.filter(sales_person_name=user_id)
            for supplier in supplier_obj:
                ...
                subscriber_data = {
                    ...
                    'total_amount': str("%0.2f" % float(total_amount)),
                    'status': status,
                    'sales_partner': supplier.sales_person_name.user_first_name + ' ' + supplier.sales_person_name.user_last_name,
                }
            ...
            for supplier in supplier_list:
                ...
                for business in business_obj:
                    try:
                        ...
                        pre_ser_obj_list = PremiumService.objects.filter(
                            business_id=str(advert_sub_obj.business_id))
                        premium_service, advert_slider, top_advert = 'NA', 'No', 'No'
                        premium_start_date, slider_start_date, top_advert_start_date = 'NA', 'NA', 'NA'
                        premium_end_date, slider_end_date, top_advert_end_date = 'NA', 'NA', 'NA'

                        for pre_ser_obj in pre_ser_obj_list:
                            if pre_ser_obj.premium_service_name != "Advert Slider" and pre_ser_obj.premium_service_name != "Top Advert":
                                if premium_service == 'NA':
                                    premium_service = ''
                                if premium_start_date == 'NA':
                                    premium_start_date = ''
                                if premium_end_date == 'NA':
                                    premium_end_date = ''
                                premium_service = premium_service + '\n\n' + pre_ser_obj.premium_service_name
                                premium_start_date = premium_start_date + '\n\n' + pre_ser_obj.start_date
                                premium_end_date = premium_end_date + '\n\n' + pre_ser_obj.end_date
                            if pre_ser_obj.premium_service_name == "Advert Slider":
                                advert_slider = 'Yes'
                                slider_start_date = pre_ser_obj.start_date
                                slider_end_date = pre_ser_obj.end_date
                        ...
                        try:
                            payment_obj = PaymentDetail.objects.get(business_id=str(advert_sub_obj.business_id))
                            if payment_obj.total_amount:
                                total_amount = str("%0.2f" % float(payment_obj.total_amount))
                            else:
                                total_amount = 0

                            if payment_obj.paid_amount:
                                paid_amount = str("%0.2f" % float(payment_obj.paid_amount))
                            else:
                                paid_amount = 0
                        except Exception as e:
                            pass
                                            ...
                                            for obj4 in category_lst4:
                                                ...
                                                if category_lst5:
                                                    for obj5 in category_lst5:
                                                        ...
                                                        advert_obj = Advert.objects.filter(
                                                            category_level_5=category_id)
                                                        advert_count = get_advert_count(advert_obj)
                                                        business_obj = Business.objects.filter(
                                                            category_level_5=category_id)
                                                        business_count = get_business_count(business_obj)
                                                        advert_data = {
...
def get_advert_count(advert_obj):
    i = 0
...
def get_business_count(business_obj):
    i = 0
    for business in business_obj:
        pre_date = datetime.now().strftime("%d/%m/%Y")
        pre_date = datetime.strptime(pre_date, "%d/%m/%Y")
        end_date = business.end_date
        start_date = datetime.strptime(business.start_date, "%d/%m/%Y")
        if start_date <= pre_date:
            end_date = datetime.strptime(end_date, "%d/%m/%Y")
            date_gap = end_date - pre_date
            if int(date_gap.days) >= 0:
...
def get_registered_consumer_data(request):
    try:
        data = {}
        final_list = []
        area = ''
        ...
        try:
            ...
            if mngt_city:
                consumer_list = consumer_list.filter(city_place_id=mngt_city)
            total_register_user = len(consumer_list.filter(consumer_type='register'))
            consumer_list = ConsumerProfile.objects.filter(date_joined__range=[from_date, to_date])
            if mngt_city:
                consumer_list = consumer_list.filter(city_place_id=mngt_city)
            total_guest_user = len(consumer_list.filter(consumer_type='guest'))

            data = {
                'success': 'true',
                'total_register_user': str(total_register_user),
                'total_guest_user': str(total_guest_user)
            }
        except Exception as e:
            print "==============Exception===============================", e
            data = {'success': 'false', 'message': 'Error in  loading page. Please try after some time'}
    ...
    try:
        ...
        try:
            if request.GET.get('mngt_city'):
                ...
                consumer_list = ConsumerProfile.objects.filter(date_joined__range=[from_date, to_date],
                                                               city_place_id=mngt_city, consumer_type='register')
    ...
    try:
        ...
        try:
            if request.GET.get('activity_city'):
                ...
                consumer_list = ConsumerProfile.objects.filter(date_joined__range=[from_date, to_date],
                                                               city_place_id=activity_city, consumer_type='register')
                n = 0
                for consumer in consumer_list:
                    n = n + 1
                    user_id = consumer.consumer_full_name
                    date = consumer.consumer_created_date.strftime('%d/%m/%Y')
    ...
    try:
        ...
        try:
            if request.GET.get('consumer_city'):
                ...
                consumer_list = ConsumerProfile.objects.filter(date_joined__range=[from_date, to_date],
                                                               city_place_id=consumer_city, consumer_type="register")
                n = 0
                for consumer in consumer_list:
                    n = n + 1
                    # user_id = consumer.consumer_id
                    consumer_email_id = consumer.consumer_full_name
                    consumer_contact_no = consumer.consumer_contact_no
...
def admin_dashboard(request):
    try:
        ...
        try:
            ...
            last_date = last_date.replace(hour=00, minute=00,second = 00)
            ...
            # New Subscribers
            total_new_subscriber = Supplier.objects.filter(
                supplier_created_date__range=[last_date, current_date]).count()
            ...
            consumer_lst = ConsumerProfile.objects.filter(last_time_login__gt=past_date,
                                                          last_time_login__lt=present_date)
            for obj in consumer_lst:
                consumer_id = obj.consumer_id

                consumer_list0 = ConsumerProfile.objects.filter(consumer_id=consumer_id, last_time_login__regex=' 00:',
                                                                consumer_type='register').count()
                count_zero = count_zero + consumer_list0

                for hour in range(0, 9):
                    hour = ' 0' + str(hour) + ':'
                    consumer_list = ConsumerProfile.objects.filter(consumer_id=consumer_id, last_time_login__regex=hour,
                                                                   consumer_type='register').count()
                ...
                for hour in range(9, 17):
                    if hour == 9:
                        ...
                        hour = ' ' + str(hour) + ':'
                    consumer_list1 = ConsumerProfile.objects.filter(consumer_id=consumer_id,
                                                                    last_time_login__regex=hour,
                                                                    consumer_type='register').count()
                ...
                for hour in range(17, 24):
                    hour = ' ' + str(hour) + ':'
                    consumer_list2 = ConsumerProfile.objects.filter(consumer_id=consumer_id,
                                                                    last_time_login__regex=hour,
                                                                    consumer_type='register').count()
            ...
            for obj in consumer_lst:
                ...
                consumer_list0 = ConsumerProfile.objects.filter(consumer_id=consumer_id, last_time_login__regex=' 0:',
                                                                consumer_type='guest').count()
            ...
            data = {'success': 'true', 'count_zero_guest': count_zero_guest, 'count_first_guest': count_first_guest,
                    'count_second_guest': count_second_guest,
                    ...
                    'count_zero': count_zero, 'count_first': count_first, 'count_second': count_second,
        ...
        try:
            ...
            if city_front == 'all':
                ...
                for obj in consumer_lst:
                    ...
                    consumer_list0 = ConsumerProfile.objects.filter(consumer_id=consumer_id,
                                                                    last_time_login__regex=' 00:',
                                                                    consumer_type='register').count()
                    count_zero1 = count_zero1 + consumer_list0

                    for hour in range(0, 9):
                        hour = ' 0' + str(hour) + ':'
                        consumer_list = ConsumerProfile.objects.filter(consumer_id=consumer_id,
                                                                       last_time_login__regex=hour,
                                                                       consumer_type='register').count()
                        count_first = count_first + consumer_list
            ...
            data = {'success': 'true', 'jan1': jan1, 'feb1': feb1, 'mar1': mar1, 'apr1': apr1, 'may1': may1,
                    ...
                    'count_zero_guest': count_zero_guest, 'count_first_guest': count_first_guest,
                    'count_second_guest': count_second_guest, 'count_third_guest': count_third_guest}
    ...
    try:
        ...
        try:
            if request.GET.get('week_var') == 'month' and request.GET.get('citys_var') != 'all':
                ...
                # New Subscribers
                total_new_subscriber_list = Supplier.objects.filter(
                    supplier_created_date__range=[last_date, current_date])
...
def admin_report(request):
    try:
        ...
        try:
            ...
            pre_date = str(dates[0]) + '/' + str(dates[1]) + '/' + str(dates[2])
            ...
            data = {'success': 'true', 'subscriber_data': subscriber_obj, 'category_data': category_obj,
...
def get_subscriber_list(request):
    ...
    if city_id == 'all':
        supplier_obj = Supplier.objects.all().order_by('business_name')
    else:
        supplier_obj = Supplier.objects.filter(city_place_id=city_id).order_by('business_name')
...
def get_category_list(request):
    city_id = request.GET.get('city_id')
    if city_id == 'all':
        category_obj = CategoryCityMap.objects.all()
    else:
        category_obj = CategoryCityMap.objects.filter(city_place_id=city_id)
            ...
            if request.GET.get('sales_nm'):
                ...
                for supp_obj in Supplier_list:
                    ...
                    for bus_obj in business_list:
                        ...
                        category = bus_obj.category.category_name
                        category_level_1 = ''
                        category_level_2 = ''
                        if bus_obj.category_level_1:
                            category_level_1 = bus_obj.category_level_1.category_name
                        if bus_obj.category_level_2:
                            category_level_2 = bus_obj.category_level_2.category_name
                        ...
                        consumer_data = {
                            ...
                            'ser_cost': "{0:.2f}".format(float(total_ser_cost)),
                            'pay_amt': "{0:.2f}".format(float(total_pay_amt)),
...
def get_new_subss_data(request):
    try:
        ...
        try:
            ...
            Supplier_list = Supplier.objects.filter(supplier_created_date__range=[from_date, to_date])
            if request.GET.get('sales_nm'):
                Supplier_list = Supplier_list.filter(sales_person_name=request.GET.get('sales_nm'))
            ...
            for supp_obj in Supplier_list:
                ...
                for bus_obj in business_list:
                    ...
                    pre_ser_obj_list = PremiumService.objects.filter(
                        business_id=str(bus_obj.business_id))
                    premium_service, advert_slider, top_advert = 'NA', 'No', 'No'
                    premium_start_date, slider_start_date, top_advert_start_date = 'NA', 'NA', 'NA'
                    premium_end_date, slider_end_date, top_advert_end_date = 'NA', 'NA', 'NA'
                    ...
                    consumer_data = {
                        ...
                        'prem_list_ser': premium_service,
                        'prem_start_date': premium_start_date,
                        'prem_end_date': premium_end_date,
                        'pre_adv_slider_part': advert_slider,
                        'adv_start_date': slider_start_date,
                        'adv_end_date': slider_end_date,
                        'pre_top_adv_part': top_advert,
                        'top_start_date': top_advert_start_date,
                        'top_end_date': top_advert_end_date,
                        'total_ser_cost': str("%0.2f" % float(total_ser_cost)),
                        'total_pay_amt': str("%0.2f" % float(total_pay_amt))
                    }
                    final_list.append(consumer_data)
            data = {'success': 'true', 'data': final_list}
        except Exception as e:
                                    ...
                                    for obj3 in category_lst3:
                                        ...
                                        if category_lst4:
                                            for obj4 in category_lst4:
                                                ...
                                                register_user_list = ConsumerProfile.objects.filter(
                                                    consumer_type='register')
                                                guest_user_list = ConsumerProfile.objects.filter(consumer_type='guest')
                                                register_like_count = AdvertView.objects.filter(
                                                ...
                                                if category_lst5:
                                                    for obj5 in category_lst5:
                                                        ...
                                                        register_user_list = ConsumerProfile.objects.filter(
                                                            consumer_type='register')
                                                        guest_user_list = ConsumerProfile.objects.filter(
                                                            consumer_type='guest')
                                                        register_like_count = AdvertView.objects.filter(
                                                            advert_id__in=advert_list5,
                                                            user_id__in=register_user_list).count()

                                                        guest_like_count = AdvertView.objects.filter(
                                                            advert_id__in=advert_list5,
                                                            user_id__in=guest_user_list).count()
...
def get_filter_data(request):
    try:
        ...
        for adve_obj in advert_obj:
            ...
            category_data = ''
            if adve_obj.category_id:
                category_data = category_data + adve_obj.category_id.category_name
            if adve_obj.category_level_1:
                category_data = category_data + ' -> ' + adve_obj.category_level_1.category_name
            if adve_obj.category_level_2:
                category_data = category_data + ' -> ' + adve_obj.category_level_2.category_name
            if adve_obj.category_level_3:
                category_data = category_data + ' -> ' + adve_obj.category_level_3.category_name
            if adve_obj.category_level_4:
                category_data = category_data + ' -> ' + adve_obj.category_level_4.category_name
            if adve_obj.category_level_5:
                category_data = category_data + ' -> ' + adve_obj.category_level_5.category_name
            advert_data = {
                'advert_id': str(i),
    ...
    try:
        ...
        try:
            ...
            for advert in advert_obj:
                ...
                category_data = ''
                if advert_sub_obj.advert_id.category_id:
                    category_data = category_data + advert_sub_obj.advert_id.category_id.category_name
                if advert_sub_obj.advert_id.category_level_1:
                    category_data = category_data + ' -> ' + advert_sub_obj.advert_id.category_level_1.category_name
                if advert_sub_obj.advert_id.category_level_2:
                    category_data = category_data + ' -> ' + advert_sub_obj.advert_id.category_level_2.category_name
                if advert_sub_obj.advert_id.category_level_3:
                    category_data = category_data + ' -> ' + advert_sub_obj.advert_id.category_level_3.category_name
                if advert_sub_obj.advert_id.category_level_4:
                    category_data = category_data + ' -> ' + advert_sub_obj.advert_id.category_level_4.category_name
                if advert_sub_obj.advert_id.category_level_5:
                    category_data = category_data + ' -> ' + advert_sub_obj.advert_id.category_level_5.category_name
                ...
                advert_data = {
                    ...
                    'uploaded_pictures': image_count,
                    'uploaded_videos': video_count,
                    'memory_usages': advert_sub_obj.advert_id.image_video_space_used,
                    'total_service_cost': str("%0.2f" % float(total_amount)),
                    'total_amount_paid': str("%0.2f" % float(paid_amount)),
                    'saleman_name': advert_sub_obj.advert_id.supplier_id.sales_person_name.user_first_name + ' ' + advert_sub_obj.advert_id.supplier_id.sales_person_name.user_last_name,
                    'saleman_number': advert_sub_obj.advert_id.supplier_id.sales_person_name.user_contact_no
                }
                final_list.append(advert_data)
    ...
    try:
        ...
        try:
            ...
            for hour in hours_list:
                hour_based_data = []
                consumer_list = ConsumerProfile.objects.filter(last_time_login__range=[past_date, present_date],
                                                               last_time_login__regex=hour, consumer_type='register')
                if city_front:
                ...
                consumer_list = ConsumerProfile.objects.filter(last_time_login__range=[past_date, present_date],
                                                               last_time_login__regex=hour, consumer_type='guest')
                if city_front:
                    consumer_list = consumer_list.filter(city_place_id=city_front)
    ...
    try:
        ...
        try:
            ...
            for cat in cat_list:
                ...
                #final_list.append([day[0],visits,'#b87333'])
                final_list.append({'cat_name':day[0],'count':visits})
    ...
    try:
        ...
        try:
            ...
            for cat in cat_list:
                business_data = {
                    'day': cat.category_name,
                    'visits': Business.objects.filter(category=cat.category_id,
        ...
        try:
            ...
            for cat in cat_list:
                ...
                for obj in business_obj:
                    try:
                        payment_obj = PaymentDetail.objects.get(business_id=obj.business_id,
                                                                payment_created_date__range=[past_date, present_date])
                        if payment_obj.paid_amount:
...
def get_advert_data(request):
    supplier_id = request.GET.get('subscriber_id')
    city_id = request.GET.get('city_id')
    advert_list = []
    advert_obj = Advert.objects.all().order_by('advert_name')
    if city_id != 'all':
        advert_obj = advert_obj.filter(city_place_id=city_id)
    if supplier_id != 'all':
        advert_obj = advert_obj.filter(supplier_id=supplier_id)
...
def get_advert_status_data(request):
    try:
        data = {}
        final_list = []
        try:
            ...
            if category_id != "all" and category_id != "":
                advert_obj = advert_obj.filter(category_id=category_id)
            ...
            for advert in advert_obj:
                ...
                advert_sub_obj = AdvertSubscriptionMap.objects.get(advert_id=str(advert.advert_id))
                try:
                    business_obj = Business.objects.get(business_id=advert_sub_obj.business_id.business_id)

                    end_date = business_obj.end_date
                    end_date = datetime.strptime(end_date, "%d/%m/%Y")
                    if from_date <= end_date <= to_date:
                        print 'true'
                        ...
                        #today_date = today_date.strftime("%Y-%m-%d")                
                            if end_date >= today_date:
                                ...
                                status = '<span style="cursor: default;" class="btn btn-danger">Expired</span>'

                            supplier_id = business_obj.supplier.supplier_id
                            business_name = advert_sub_obj.id
                            start_date = business_obj.start_date
                            end_date = business_obj.end_date
                            #advert_sub_obj = AdvertSubscriptionMap.objects.get(business_id=str(business_obj.business_id))
                            advert_obj = Advert.objects.get(advert_id=advert_sub_obj.advert_id.advert_id)
                            advert_id = advert_obj.advert_id
                            advert_name = advert_obj.advert_name
                            updated_by = advert_obj.updated_by
                            category_data = ''
                            if advert_obj.category_id:
                                category_data = category_data + advert_obj.category_id.category_name
                            if advert_obj.category_level_1:
                                category_data = category_data + ' -> ' + advert_obj.category_level_1.category_name
                            if advert_obj.category_level_2:
                                category_data = category_data + ' -> ' + advert_obj.category_level_2.category_name
                            if advert_obj.category_level_3:
                                category_data = category_data + ' -> ' + advert_obj.category_level_3.category_name
                            if advert_obj.category_level_4:
                                category_data = category_data + ' -> ' + advert_obj.category_level_4.category_name
                            if advert_obj.category_level_5:
                                category_data = category_data + ' -> ' + advert_obj.category_level_5.category_name
                            print "i:::",i
