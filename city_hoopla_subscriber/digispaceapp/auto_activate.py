...
def auto_activate_citystar():
    #print '---------activation date--------',datetime.now().strftime("%d/%m/%Y")
    print '---------activation date--------',datetime.now().strftime("%Y-%m-%d")
    try:
        ...
        #act_date = datetime.now().strftime("%Y-%m-%d 00:00:00")

        #star_obj = CityStarDetails.objects.filter(start_date__range=act_date)
        star_obj = CityStarDetails.objects.filter(start_date__range=td)
