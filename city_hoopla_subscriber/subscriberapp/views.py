    ...
    if not request.user.is_authenticated():
        ...
        for coupons in coupon_user:
            ...
            coupon_obj = {
                ...
                'user_id': str(coupons.user_id.consumer_id),
...
def edit_subscription(request):
    if not request.user.is_authenticated():
        ...
        if business_obj.duration == '3':
            basic_amount = int(rate_obj.cost_for_3_days)
        if business_obj.duration == '7':
            basic_amount = int(rate_obj.cost_for_7_days)
        if business_obj.duration == '30':
            basic_amount = int(rate_obj.cost_for_30_days)
        if business_obj.duration == '90':
            basic_amount = int(rate_obj.cost_for_90_days)
        if business_obj.duration == '180':
            basic_amount = int(rate_obj.cost_for_180_days)
        ...
        try:
            if request.GET.get('advert_var') != '':
                if request.GET.get('week_var') == 'month':
                    ...
                    today_date = str(datetime.now())
                    one_month_date = str(datetime.now() - timedelta(days=30))
                    Advert_list = Advert.objects.filter(supplier_id=request.GET.get('supplier_id'),advert_id=request.GET.get('advert_var'))
                    ...
                    for i in range(0, numb):
                        if i == 0:
                            ...
                            coupon_code_count5 = CouponCode.objects.filter(
                                creation_date__range=[start_date, end_date],advert_id=request.GET.get('advert_var')).count()
                            print "------Fifth week"
                ...
                if request.GET.get('week_var') == 'week':
                    ...
                    list = []
                    total_view_list = CouponCode.objects.filter(creation_date__range=[last_date, current_date],advert_id=request.GET.get('advert_var'))
                    mon1 = tue1 = wen1 = thus1 = fri1 = sat1 = sun1 = 0
                    if total_view_list:
                        for view_obj in total_view_list:
                            creation_date = view_obj.creation_date
                    ...
                    list = []
                    total_view_list = AdvertView.objects.filter(creation_date__range=[last_date, current_date],advert_id=request.GET.get('advert_var'))
                    mon2 = tue2 = wen2 = thus2 = fri2 = sat2 = sun2 = 0
                    if total_view_list:
                        for view_obj in total_view_list:
                            creation_date = view_obj.creation_date
