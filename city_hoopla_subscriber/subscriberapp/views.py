            ...
            if request.GET.get('week_var') == 'month':
                ...
                for i in range(0,numb):
                    if i==0:
                        start_date = date(today.year,today.month,01)
                        end_date = date(today.year,today.month,8) 
                        advert_total_count1 = AdvertView.objects.filter(creation_date__range=[start_date,end_date]).count()
                    elif i==1:
                        start_date = date(today.year,today.month,8)
                        end_date = date(today.year,today.month,16) 
                        advert_total_count2 = AdvertView.objects.filter(creation_date__range=[start_date,end_date]).count()
                    elif i==2:
                        start_date = date(today.year,today.month,16)
                        end_date = date(today.year,today.month,23) 
                        advert_total_count3 = AdvertView.objects.filter(creation_date__range=[start_date,end_date]).count()
                    elif i==3:
                        start_date = date(today.year,today.month,23)
                        end_date = date(today.year,today.month,30) 
                        advert_total_count4 = AdvertView.objects.filter(creation_date__range=[start_date,end_date]).count()
                    elif i==4:
                        start_date = date(today.year,today.month,30)
                        end_date = date(today.year,today.month,31) 
                        advert_total_count5 = AdvertView.objects.filter(creation_date__range=[start_date,end_date]).count()
    ...
    try:
        ...
        #########################advert_views_total#############################
            ...
            for advert_obj in Advert_list1:
                print advert_obj
                advert_id = advert_obj.advert_id
                advert_views = Advert.objects.filter(advert_id=advert_id,creation_date__range=[start_date,end_date]).count()
                thumbs_count = AdvertLike.objects.filter(advert_id=advert_id,creation_date__range=[start_date,end_date]).count()
                shares_count = AdvertShares.objects.filter(advert_id=advert_id,creation_date__range=[start_date,end_date]).count()
            ...
            data = {'success':'true','advert_nm':advert_nm,'supplier_id':supplier_id,'logo':logo,'advert_views_total':advert_views_total,'thumbs_count_total':thumbs_count_total,'shares_count_total':shares_count_total,'jan1':jan1,'feb1':feb1,'mar1':mar1,'apr1':apr1,'may1':may1,'jun1':jun1,'jul1':jul1,'aug1':aug1,'sep1':sep1,'oct1':oct1,'nov1':nov1,'dec1':dec1,
